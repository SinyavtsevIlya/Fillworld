...
namespace UniRx
{
    ...
    public class ReactiveCollection<T> : Collection<T>, IReactiveCollection<T>, IDisposable
    {
        ...
        {
            if (collectionBeforeReset != null) collectionBeforeReset.OnNext(Unit.Default);
        ...
        [NonSerialized]
        Subject<Unit> collectionBeforeReset = null;
        public IObservable<Unit> ObserveBeforeReset()
        {
            if (isDisposed) return Observable.Empty<Unit>();
            return collectionBeforeReset ?? (collectionBeforeReset = new Subject<Unit>());
        }
